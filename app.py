...
# On Streamlit Cloud, you should set this in the app's secrets.
GOOGLE_API_KEY = st.secrets.get("GOOGLE_API_KEY", os.getenv("GOOGLE_API_KEY"))

if not GOOGLE_API_KEY:
    st.error("❌ GOOGLE_API_KEY not found. Please set it in your Streamlit secrets or a .env file.")
    st.stop()

genai.configure(api_key=GOOGLE_API_KEY)


# --- UPDATED FUNCTION TO HANDLE RATE LIMITING ---
def get_vector_store(text_chunks):
    """
    Creates a FAISS vector store from text chunks in batches to handle API rate limits.
    """
    if not text_chunks:
        st.warning("⚠️ No text chunks to process.")
        return

    try:
        embeddings = GoogleGenerativeAIEmbeddings(model="models/embedding-001")
        
        # Process in smaller batches to stay within free tier limits
        batch_size = 20  # You can adjust this number
        vector_store = None
        
        with st.spinner("Creating embeddings in batches... This might take a moment."):
            for i in range(0, len(text_chunks), batch_size):
                ...
                # Log progress to the console/logs
        ...
        print(f"Error details: {e}") # Also print to logs for debugging

# --- THE REST OF YOUR CODE (with minor improvements) ---

# Theme settings
if "dark_mode" not in st.session_state:
    st.session_state.dark_mode = False

# Using a toggle button for a cleaner look
...
def generate_pdf_report(qa_history):
    filename = f"qa_history_{uuid.uuid4().hex}.pdf"
    doc = SimpleDocTemplate(filename, pagesize=letter)
    styles = getSampleStyleSheet()
    story = [Paragraph("<b>Q&A History</b>", styles["Title"]), Spacer(1, 12)]
    
    for index, (question, answer) in enumerate(qa_history, start=1):
        q_para = Paragraph(f"<b>Question {index}:</b> {question}", styles["Normal"])
        a_para = Paragraph(f"<b>Answer {index}:</b> {answer.replace(chr(10), '<br/>')}", styles["Normal"])
        story.extend([q_para, Spacer(1, 6), a_para, Spacer(1, 12)])
...
def process_question(question):
    ...
    if question.lower().strip() == st.session_state.last_question.lower().strip():
        st.warning("⚠️ You've just asked this question. Please try a different one.")
        return
    ...
    try:
        ...
        st.error(f"❌ FAISS Index not found or failed to load. Please process your PDFs first. Error: {e}")
        return
        
    docs = new_db.similarity_search(question, k=5)
    if not docs:
        st.error("⚠️ No relevant context found. Try reprocessing PDFs or asking a different question.")
        return
    ...
    st.rerun() # Rerun to update the history display immediately
...
def main():
    ...
    with st.sidebar:
        ...
        pdf_docs = st.file_uploader("📂 Upload PDFs & Process", accept_multiple_files=True, type="pdf")
        ...
        if st.button("Submit & Process"):
            if not pdf_docs:
                ...
                if not raw_text.strip():
                    st.error("❌ No text extracted. The PDF might be image-based or corrupted.")
                else:
                    text_chunks = get_text_chunks(raw_text)
                    get_vector_store(text_chunks) # Call the updated function

        st.markdown("---")
        if st.session_state.qa_history:
            st.subheader("Download Q&A")
            pdf_file = generate_pdf_report(st.session_state.qa_history)
